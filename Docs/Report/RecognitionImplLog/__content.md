...
(이하, 깃허브에서 라텍 보이도록 이미지 삽입 ...)

![](2020-09-17-03-03-07.png)

계산 과정에서 실수가 없었다면, 잘 동작할 것입니다.

**경과**

- **0918.01:03** 어제 하루 무아지경으로 코딩에 몰두한 결과, 구와 구 사이의 물리 시뮬레이션을 꽤 안정적으로 구현할 수 있었습니다. 시뮬레이션은 앞서 언급한대로 시분할이 아닌 Swept 검사를 통해 충돌 이벤트가 존재할 때만 시간을 진행시키는 이벤트 주도적인 방식으로 동작하기 때문에, 시뮬레이션 기간이 늘어날수록 선형적으로 연산량이 증가하는 시분할 방식에 비해 매우 경제적입니다.  <br> ![](GIF%202020-09-18%20오전%201-07-02.gif)
> [이 링크](https://sites.google.com/site/3dgameprogram/home/physics-modeling-for-game-programming/-gibongaenyeomdajigi---mulli/-jiljeom-ui-chungdol)를 참고해 충돌 검출뿐 아니라 충돌 이후의 탄성 계수를 고려한 피드백까지도 제법 그럴싸하게 구현할 수 있었습니다. 
- **0918.01:33** (아이디어) 당구공이 작아서 그런 건지, 당구공 표면의 왁스가 말라서 그런건지, 천 상태가 엉망이라 그런건지, 이 모든게 원인인지 .... 잘 모르겠지만, 공을 어떻게 쳐도 밀어치기로 들어가는 경향이 있습니다. 한 가지 다행인 점은 진행 방향과 구르는 방향이 어긋나는 일은 없다는 점이라, 충돌 시점에 새로운 속도를 계산할 때 기존 속도에 약간의 가중치를 주는 것으로 어느 정도 모델을 근사할 수 있을 것으로 보인다는 점입니다. (*공을 어떻게 치더라도 밀어치기가 되는 모델입니다. 다만, 충돌 시점에 속도에 이를 반영하는 식으로 구현하므로 황오시는 어렵겠네요*)
- **0918.09:51** 평면과 구 궤적 사이의 충돌 검출 로직을 구현합니다. 구 궤적은 항상 평면과 가장 가까운 점(즉, 구 중심에서 평면에 내린 수선과 구의 교점)에서 접하기 때문에, 해당 접점과 평면에 대한 궤적 검사를 하면 어렵지 않게 구할 수 있을 겁니다.  
![](2020-09-18-10-21-23.png)
- **0918.11:46** 계속해서 동작을 안하길 30분, 계산 실수를 발견했습니다.  
![](2020-09-18-11-47-34.png)
- **0918.20:47** 커다란 실수를 발견했습니다. 매 계산에서 항상 $e^at$의 원점을 0으로 놓고 생각해서, 델타 시간이 달라질 때마다 시뮬레이션의 결과가 달라지는 문제가 발생했습니다. 
- **0919.16:21** 그런 문제가 아닌 것으로 밝혀졌습니다. 단순히 조건 검사 하나를 깜빡해서 눈에 안 보이는 것뿐이었습니다. 감쇠 함수 f(t)= e^(-at)에 대해, f(T0) * f(t-T0) = f(t)라는 말뜻을 이제야 이해했네요 ... 
그 결과, 이렇게 동작합니다. [유튜브 링크](https://youtu.be/_j6dDIikoDM)  
아직까지는 쿠션에 대한 동작이 별로 정확하지 않은데, 수많은 실험 결과 쿠션의 횡방향 속도는 공의 속도가 빠를 땐 거의 줄어들지 않고, 공의 속도가 느릴 땐 매우 많이 줄어든느 것을 발견했습니다. 이를 바탕으로 적당한 휴리스틱을 집어넣을 계획입니다.
- **0919.16:30** 방금 발견한 사실인데, 쿠션의 각도는 꼭 속도에 반비례하는 것은 아닌 것 같습니다. 오히려 낮은 속도로 치면 쿠션을 여러 번 왕복해도 반사각이 그다지 좁아지지 않습니다. 반사각이 가장 크게 줄어드는 시점은 강하게 때린 공이 장 쿠션을 왕복하는 상황인데, 이로 미루어보아 당구공의 회전을 아예 고려하지 않을 수는 없을 듯합니다.  
다만 성능을 위해 시분할 모델이 아닌 결정론적 모델을(제가 지은 이름입니다) 활용하기 때문에 토크나 회전 마찰 등을 자세하게 고려하기는 어렵고, **마지막 충돌 이후로 흐른 시간, 현재 속도, 회전 방향**등을 종합적으로 고려한 휴리스틱을 작성합니다.

## 200921-1329

작품이 어느 정도 궤도에 올랐습니다. 이제부턴 사소한 개선의 연속일 것으로 보입니다.

이 일지의 업데이트는 여기서 중단합니다.
