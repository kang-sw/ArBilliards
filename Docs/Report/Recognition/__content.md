...
**상세**  
중점 후보를 임의로 선정해 최대 적합 후보를 찾는 방법은 지나치게 불안정해 다른 방법을 우선적으로 검토해 봅니다.  
앞서 프로토타이핑을 위해 단순히 non-table contour의 무게중심을 계산하는 방법으로 공의 중점을 구했었는데, 이 방법은 제법 안정적이었지만 ![](GIF%202020-09-11%20오전%2011-13-10.gif) 다음과 같이 공의 경계선이 겹칠 때 구별하지 못하는 문제가 있었습니다.  
![](GIF%202020-09-13%20오후%2011-13-35.gif)  
이 문제를 해결하는 방향으로 다음 목표를 잡습니다.  

![](2020-09-13-23-41-30.png)  
공이 겹칠 때 에지는 위와 같은 모양을 띠는데, 일차적으로 침식 연산을 수십 회 반복하면 두 개의 중심을 얻을 수 있을 것으로 기대됩니다. 핵심은 겹친 덩어리를 다수의 convex hull로 분리하는 것으로, 침식에 의한 방법이 효과적으로 동작하지 않는다면 contour 수준에서 다수의 convex hull을 뽑아내는 방법을 연구해봐야 하겠습니다.
