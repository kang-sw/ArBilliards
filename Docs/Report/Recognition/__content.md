...
- **0918.01:33** (아이디어) 당구공이 작아서 그런 건지, 당구공 표면의 왁스가 말라서 그런건지, 천 상태가 엉망이라 그런건지, 이 모든게 원인인지 .... 잘 모르겠지만, 공을 어떻게 쳐도 밀어치기로 들어가는 경향이 있습니다. 한 가지 다행인 점은 진행 방향과 구르는 방향이 어긋나는 일은 없다는 점이라, 충돌 시점에 새로운 속도를 계산할 때 기존 속도에 약간의 가중치를 주는 것으로 어느 정도 모델을 근사할 수 있을 것으로 보인다는 점입니다. (*오시는 되는데, 황오시는 안 된다는 뜻입니다*)
